...
from tinyloom.core.config import Config, CompactionConfig, ModelConfig
...
def _cfg(context_window=200_000, threshold=0.8, strategy="summarize", **comp_kw) -> Config:
    return Config(
        model=ModelConfig(context_window=context_window),
        compaction=CompactionConfig(threshold=threshold, strategy=strategy, **comp_kw),
    )
...
async def test_maybe_compact_returns_none_below_threshold():
    ...
    result = await maybe_compact(provider, messages, _cfg())
...
async def test_truncate_strategy():
    ...
    result = await maybe_compact(provider, messages, _cfg(strategy="truncate"))
...
async def test_maybe_compact_passes_config_to_summarize():
    ...
    with patch("tinyloom.providers.create_provider", return_value=compact_provider):
        result = await maybe_compact(default_provider, messages, config)
    assert result is not None
